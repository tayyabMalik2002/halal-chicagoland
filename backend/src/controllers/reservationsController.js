...
const createReservation = async (req, res) => {
  ...
  const [customerRows] = await pool.query('SELECT customer_id FROM customers WHERE customer_id = ?', [customer_id]);
  ...
  const [rows] = await pool.query('SELECT * FROM reservations WHERE reservation_id = ?', [result.insertId]);
  res.status(201).json({ data: rows[0] });
};

const updateReservation = async (req, res) => {
  const { id } = req.params;
  const [existing] = await pool.query('SELECT * FROM reservations WHERE reservation_id = ?', [id]);
  ...
  await pool.query(
    `UPDATE reservations
     SET reservation_date = ?, reservation_time = ?, party_size = ?, status = ?, special_requests = ?
...
const cancelReservation = async (req, res) => {
  ...
  await pool.query('UPDATE reservations SET status = ? WHERE reservation_id = ?', ['cancelled', id]);
  const [rows] = await pool.query('SELECT * FROM reservations WHERE reservation_id = ?', [id]);
  res.json({ data: rows[0] });
};
...
const deleteReservation = async (req, res) => {
  const { id } = req.params;
  const [existing] = await pool.query('SELECT reservation_id FROM reservations WHERE reservation_id = ?', [id]);
  if (!existing.length) throw ApiError.notFound(`Reservation ${id} not found.`);
